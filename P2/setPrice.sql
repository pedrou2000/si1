...
FROM (SELECT ROUND((products.price / (1 + (EXTRACT(YEAR FROM CURRENT_DATE) - (EXTRACT(YEAR FROM orders.orderdate))) * 0.02)), 2) AS price, orderdetail.orderid AS orderid, orderdetail.prod_id AS prod_id
