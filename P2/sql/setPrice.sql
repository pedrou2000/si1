...
FROM (SELECT ROUND(CAST((products.price / (1 + (EXTRACT(YEAR FROM CURRENT_DATE) - (EXTRACT(YEAR FROM orders.orderdate))) * 0.02)) AS numeric), 2)*orderdetail.quantity AS price, orderdetail.orderid AS orderid, orderdetail.prod_id AS prod_id
